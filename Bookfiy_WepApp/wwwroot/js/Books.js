$(document).ready(function () {
    ...
    datatable = $('#Books').DataTable({
        ...
        columns: [
            ...
            {
                "name": "PublishingDate",
                "render": function (data, type, row) { return moment(row.publishingDate).format('ll') }
            },
            { "data": "hall", "name": "Hall" },
            { "data": "categories", "name": "Categories", "orderable": false },
            {
                "name": "IsAvailabbleForRent", "render": function (data, type, row) {
                    return `<span class="badge badge-light-${(row.isAvailabbleForRent ? 'success' : 'danger')} ">
                                                           ${(row.isAvailabbleForRent ? 'Available' : 'Not Available')}
                                        </span>`;
                }
            },
            {
                "name": "IsDelete", "render": function (data, type, row) {
                    return `<span class="badge badge-light-${(row.isDelete ? 'danger' : 'success')} js-status">
                                                                           ${(row.isDelete ? 'Deleted' : 'Available')}
                                                </span>`;
                }
            },
            {
